 ...
 *  and use that
 */
const advance = function (reference) {
    const now = exports.timestamp.make();
    if (!reference || !_.is.String(reference) || (reference < now)) {
        return now;
    }

    const rdate = new Date(reference);
 ...
 */
const check_values = function(otimestamp, ntimestamp) {
 ...
 */
const check_dictionary = function(od, nd, paramd)  {
    ...
    const ntimestamp = nd[paramd.key];
    const otimestamp = od[paramd.key];
 ...
 */
const add_timestamp = function(d, paramd)  {
    paramd = _.defaults(paramd, {
        key: '@timestamp',
 ...
 *  was updated.
 */
const update_timestamp = function(d, paramd)  {
    paramd = _.defaults(paramd, {
        key: '@timestamp',
